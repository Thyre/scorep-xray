...
bool
FilterConverter::writeFile( std::string& outFilePath, std::string& content )
{
...
bool
FilterConverter::determineInputFormat()
{
    bool xray   = false;
    bool scorep = false;
...
bool
FilterConverter::saveAsScorep( std::string& outFilePath )
{
    if ( !formatOkay )
    {
...
bool
FilterConverter::saveAsXRay( std::string& outFilePath )
{
    if ( !formatOkay )
    {
        // Better to fail than to risk undefined instrumentation behaviour
